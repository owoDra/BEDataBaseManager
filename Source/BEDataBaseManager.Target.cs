...
public class BEDataBaseManagerTarget : TargetRules
{
	public BEDataBaseManagerTarget(TargetInfo Target) : base(Target)
	{
		Type = TargetType.Game;

		ExtraModuleNames.AddRange(new string[] { "BEDataBaseManager" });

		BEDataBaseManagerTarget.ApplySharedBETargetSettings(this);
	}
	...
	internal static void ApplySharedBETargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			BEDataBaseManagerTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Logger.LogWarning("BEDataBaseManager_EOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
	}
